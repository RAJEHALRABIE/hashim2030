...
function formatNumber(num,// تخزين محلي
const STORAGE_KEY = 'thrivve-tracker-v4.2-state'; 
// الدوال المساعدة الأساسية المُدمجة
function isValidNumber(value) {
  return typeof value === 'number' && isFinite(value) && !isNaN(value);
...
// 📌 [التصحيح النهائي لمنطق الأسبوع]
function defaultState() {
  const now = new Date();
  const monday = getMondayOfWeek(now);
  const sunday = new Date(monday);
  
  // ✅ التصحيح: إضافة 6 أيام لضمان الأحد 23/11/2025
  ...
  return {
    weekStart: monday.toISOString(), // 17/11/2025
    weekEnd: sunday.toISOString(), // 23/11/2025
...
function updateUI() {
  ...
  if (currentDayInfo) {
    ...
    // 📌 [التصحيح النهائي لعداد الأيام]
    // نحدد نهاية اليوم الأحد (23/11/2025)
    let finalWeekEnd = new Date(state.weekEnd);
    finalWeekEnd.setHours(23, 59, 59, 999);
    
    // إذا كانت نهاية الأسبوع يوم الأحد، نستخدمها مباشرة
    // إذا كانت نهاية الأسبوع المعروضة هي الإثنين (24/11)، نقوم بطرح يوم واحد للعرض
    const endDayOfWeek = new Date(state.weekEnd).getDay(); // 0 for Sunday
    if (endDayOfWeek !== 0) { // If it's not Sunday (it was Monday in your case)
        finalWeekEnd = new Date(finalWeekEnd.getTime() - (24 * 60 * 60 * 1000));
    }


    const diffMs = finalWeekEnd - today;
...
document.addEventListener('DOMContentLoaded', () => {
    // التأكد من وجود العناصر قبل محاولة ربط الأحداث
    const foundElements = navItems.length > 0 && newWeekBtn && document.getElementById('sheetKeypad');
    if (foundElements) {
        migratePeakFlagsOnce(); 
        // استدعاء updateUI مباشرة للتأكد من الرندر في حال فشل debounce
        updateUI(); 
    } else {
